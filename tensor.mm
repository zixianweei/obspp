...
- (BOOL)toBytes:(void **)data
  ...
  if (targetBufferLen != sourceBufferLen) {
    CUTENN_LOG_ERROR("{}: buffer of source({}) and target({}) is not same",
...
+ (int)elementSize:(cute::Format)format {
  ...
  CUTENN_LOG_CRITICAL("{}: elementSize unreachable", __func__);
  return 0;
}
...
Tensor::Tensor() { impl_ = [[CuteTensorImpl alloc] init]; }
...
bool Tensor::fromBytes(const void *data, const std::vector<int> &shape,
                       Format format) {
  return [impl_ fromBytes:data shape:shape format:format];
}

bool Tensor::toBytes(void **data, const std::vector<int> &shape,
                     Format format) {
  return [impl_ toBytes:data shape:shape format:format];
}

std::vector<int> Tensor::Shape() const { return [impl_ shape]; }
